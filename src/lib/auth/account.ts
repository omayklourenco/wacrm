...
import {
  ...
  isAccountPlatformStatus,
  type AccountPlatformStatus,
} from "./account-status";
...
export interface AvailableAccount {
  ...
  name: string;
  role: AccountRole;
  /** Platform-level status (Ciclo 003-R). Defaults to active if absent. */
...
export interface AccountContext {
  ...
  /** Platform status of the active account. */
  platformStatus: AccountPlatformStatus;
}
...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  const operable = accounts.filter(
    (a) => (a.platform_status ?? "active") !== "suspended",
  );

  // Pick the stored active membership if it is operable; otherwise fall
  // back to another operable org. If EVERY membership is suspended,
  // surface AccountSuspendedError so the UI can show the suspended page
  // while still listing orgs in availableAccounts for awareness.
  let active = accounts.find(
    (a) => a.is_active && (a.platform_status ?? "active") !== "suspended",
  );
  if (!active) {
    active = operable[0];
    if (active) {
  ...
  const availableAccounts: AvailableAccount[] = accounts.flatMap((a) => {
    if (!isAccountRole(a.role)) return [];
    const platformStatus: AccountPlatformStatus = isAccountPlatformStatus(
      a.platform_status,
    )
      ? a.platform_status
      : "active";
  ...
  if (!active) {
    // All memberships suspended — pick the first for the error payload.
    const suspendedId = accounts[0]?.account_id ?? "";
    throw new AccountSuspendedError(suspendedId);
  }
...
export async function requireAccountContext(
  ...
  if (ctx.platformStatus === "suspended") {
    throw new AccountSuspendedError(ctx.accountId);
  }
  return ctx;
}
