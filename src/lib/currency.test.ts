...
/** Locale-tolerant thousands grouping: en uses 1,234; pt-BR uses 1.234. */
function expectGrouped1234(out: string) {
  expect(out.includes("1,234") || out.includes("1.234")).toBe(true);
}
...
describe("formatCurrency", () => {
  ...
  it("never throws on a structurally invalid code (no DB CHECK on deals.currency)", () => {
    for (const bad of ["United States", "US", "USDD", "12", "u$d"]) {
      ...
      expectGrouped1234(formatCurrency(1234, bad));
    }
  });
