...
describe("mondayIndex", () => {
  it("maps Monday → 0 and Sunday → 6", () => {
    // Local noon constructors avoid UTC-midnight shifting the calendar day
    // in timezones west of UTC (e.g. America/Caracas).
    expect(mondayIndex(new Date(2026, 4, 18, 12))).toBe(0); // Mon
    expect(mondayIndex(new Date(2026, 4, 19, 12))).toBe(1); // Tue
    expect(mondayIndex(new Date(2026, 4, 23, 12))).toBe(5); // Sat
    expect(mondayIndex(new Date(2026, 4, 24, 12))).toBe(6); // Sun
  });

  it("aligns with DOW_SHORT_MON_FIRST labels", () => {
    expect(DOW_SHORT_MON_FIRST[mondayIndex(new Date(2026, 4, 18, 12))]).toBe(
      "Mon",
    );
    expect(DOW_SHORT_MON_FIRST[mondayIndex(new Date(2026, 4, 24, 12))]).toBe(
      "Sun",
    );
  });
});
